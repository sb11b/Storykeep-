...
The orange Grok bubble (bottom-right, draggable) asks about the open article. **Add to StoryKeep notes** writes an overlay addition, never an original vault file. Chat uses `XAI_API_KEY` on Railway only.
