...
Junior shared chat-memory tables are created on API boot from `backend/migrations/001_junior_memory.sql` then `002_junior_projects.sql`. The image copies that folder to `/app/migrations` so `_create_schema` can apply them. Uses the same `DATABASE_URL=${{Postgres.DATABASE_URL}}` as the rest of the app (no extra DB URL). To apply them yourself against the Railway plugin:
