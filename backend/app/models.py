...
class User(Base):
    ...
    fastmail_mail: Mapped["FastmailMailAccount | None"] = relationship(back_populates="user", uselist=False)
...
class GrokMessage(Base):
    ...
    body_iv: Mapped[str | None] = mapped_column(Text, nullable=True)
    body_ct: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class FastmailMailAccount(Base):
    """Fastmail JMAP API token. Ciphertext only — never the account password, never the browser, never logs."""

    __tablename__ = "fastmail_mail_accounts"
    ...
    user: Mapped[User] = relationship(back_populates="fastmail_mail")


class CursorAgentWatch(Base):
    """One row per Cloud Agent started from a Junior chat, until the follow-up is posted."""

    __tablename__ = "cursor_agent_watches"
    __table_args__ = (UniqueConstraint("agent_id", name="cursor_agent_watches_agent_id_key"),)
    ...
    agent_url: Mapped[str] = mapped_column(Text, nullable=False, default="")
    starting_branch: Mapped[str] = mapped_column(Text, nullable=False, default="main", server_default="main")
