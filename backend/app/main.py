...
def _create_schema() -> None:
    ...
    _try_sql("ALTER TABLE articles ADD COLUMN IF NOT EXISTS pinned BOOLEAN NOT NULL DEFAULT FALSE")
    _try_sql("ALTER TABLE folders ADD COLUMN IF NOT EXISTS pinned BOOLEAN NOT NULL DEFAULT FALSE")
    _try_sql("ALTER TABLE grok_conversations ADD COLUMN IF NOT EXISTS pinned BOOLEAN NOT NULL DEFAULT FALSE")
    _try_sql("ALTER TABLE articles ADD COLUMN IF NOT EXISTS pinned_at TIMESTAMPTZ")
    _try_sql("ALTER TABLE folders ADD COLUMN IF NOT EXISTS pinned_at TIMESTAMPTZ")
    _try_sql("ALTER TABLE grok_conversations ADD COLUMN IF NOT EXISTS pinned_at TIMESTAMPTZ")
    _try_sql("UPDATE articles SET pinned_at = updated_at WHERE pinned IS TRUE AND pinned_at IS NULL")
    _try_sql("UPDATE folders SET pinned_at = created_at WHERE pinned IS TRUE AND pinned_at IS NULL")
    _try_sql("UPDATE grok_conversations SET pinned_at = updated_at WHERE pinned IS TRUE AND pinned_at IS NULL")
