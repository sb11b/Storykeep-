...
class GrokConversationTests(unittest.TestCase):
    ...
    def test_parse_sse_chunk_array_and_text_deltas(self):
        array_raw = '{"choices":[{"delta":{"content":[{"type":"text","text":"Hi"},{"type":"text","text":" there"}]}}]}'
        text, active = _parse_sse_chunk(array_raw)
        self.assertEqual(text, "Hi there")
        self.assertTrue(active)
        text_raw = '{"choices":[{"delta":{"text":"Nope"}}]}'
        text, active = _parse_sse_chunk(text_raw)
